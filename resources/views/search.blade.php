...
@section('title_meta_info')
    <title>{{ $tvOrMovie['title'] }} | {{ config('app.name') }}</title>
    <meta name="description" content="Learn more about the movie {{ $tvOrMovie['title'] }}" />
    <link rel="canonical" href="{{ route('search.show', [$type, $tvOrMovie['id']]) }}" />
@endsection
...
@section('content')
    <div class="px-8 py-8">
        <div class="container mx-auto">
            ...
            <span class="capitalize tracking-wider text-gray-600">{{ $tvOrMovie['title'] }}</span>
        </div>
    </div>
    ...
    <div class="my-16 px-8">
        <div class="container mx-auto">
            <div class="flex flex-col items-start gap-8 md:flex-row">
                <img src="{{ $tvOrMovie['poster_path'] }}" alt="{{ $tvOrMovie['title'] }}"
                    class="mx-auto block w-full rounded-md sm:w-1/2" />

                <div>
                    <h1 class="text-xl font-bold capitalize tracking-wider text-gray-800 md:text-3xl">
                        {{ $tvOrMovie['title'] }}
                    </h1>
                    ...
                    <div class="mt-2 flex flex-wrap items-center text-sm text-gray-600">
                        ...
                        <span class="ml-1">{{ $tvOrMovie['vote_average'] }}</span>
                        <span class="mx-2">|</span>
                        <span>{{ $tvOrMovie['release_date'] }}</span>
                        <span class="mx-2">|</span>
                        <span>{{ $tvOrMovie['genres'] }}</span>
                    </div>

                    <p class="mt-8 text-sm leading-6 tracking-wider text-gray-600 lg:text-base lg:leading-8">
                        {{ $tvOrMovie['overview'] }}</p>
                    ...
                    <div class="my-12">
                        ...
                        <div
                            ...
                            @foreach ($tvOrMovie['cast'] as $cast)
    ...
    <div class="px-8 py-16">
        <div class="container mx-auto">
            ...
            <div class="mt-6 grid grid-cols-2 gap-8 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                @foreach ($tvOrMovie['crew'] as $crew)
